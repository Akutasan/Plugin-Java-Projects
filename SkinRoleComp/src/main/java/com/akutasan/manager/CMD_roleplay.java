            ...
            if (cmd.getName().equalsIgnoreCase("roleplay")) {
                if (args.length == 1 && args[0].equalsIgnoreCase("start")) {
                    if (this.cooldown.containsKey(p.getUniqueId())) {
                        ...
                        String[] split = Objects.requireNonNull(Main.getInstance().getFileManager().getConfiguration().getString("messageR:")).split(" ");
                        ...
                        for (int i = 0; i < split.length; i++) {
                            ...
                            if (str.equalsIgnoreCase("%componentR%")) {
                                if (i != split.length) {
                                    TextComponent comp = new TextComponent(ChatColor.translateAlternateColorCodes('&', split[i].replace("%componentR%", Main.getInstance().getFileManager().getConfiguration().getString("componentR:") + " ")));
