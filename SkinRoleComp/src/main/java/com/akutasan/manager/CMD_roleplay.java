...
public class CMD_roleplay implements CommandExecutor {
            ...
            if (cmd.getName().equalsIgnoreCase("roleplay")) {
                if (args.length == 1 && args[0].equalsIgnoreCase("start")) {
                    if (this.cooldown.containsKey(p.getUniqueId())) {
                        ...
                        String[] split = Objects.requireNonNull(SkinRoleComp.getInstance().getFileManager().getConfiguration().getString("messageR:")).split(" ");
                        ...
                        for (int i = 0; i < split.length; i++) {
                            ...
                            if (str.equalsIgnoreCase("%componentR%")) {
                                if (i != split.length) {
                                    TextComponent comp = new TextComponent(ChatColor.translateAlternateColorCodes('&', split[i].replace("%componentR%", SkinRoleComp.getInstance().getFileManager().getConfiguration().getString("componentR:") + " ")));
                                ...
                                components.put(i, new TextComponent(ChatColor.translateAlternateColorCodes('&', split[i]).replace("%player%", p.getName()).replace("%componentR%", "") + " "));
                            } else {
                                components.put(i, new TextComponent(ChatColor.translateAlternateColorCodes('&', split[i]).replace("%player%", p.getName()).replace("%componentR%", "")));
                            }
                        }
                    ...
                    p.sendMessage(ChatColor.RED + "Usage: /roleplay start");
                }
            }
        } else {
